...
export default{
  ...
  // 记录搜索历史
  addHistory (state, searchList) {
    const isInLists = state.searchLists.some(list => {
      return list.text === searchList.text
    })
    if (isInLists) {
    } else {
      state.searchLists.push(searchList)
      window.localStorage.setItem('search-lists', JSON.stringify(state.searchLists))
    }
  },
  // 清空搜索历史
  clearHistory (state) {
    window.localStorage.removeItem('search-lists')
    state.searchLists = []
  },
  // 登录/注册成功
